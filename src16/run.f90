...
*****************************************************************
      SUBROUTINE EQ_CHEMISTRY(Temp, Pres, sr_elements, sr_abunds, 
     > sr_Mpl, sr_Rpl, sr_Npoints, sr_Tfast,sr_nHmin,sr_nHmax,sr_gamma,
     > sr_model_eqcond, sr_pick_mfrac,sr_phyllosilicates,
     > sr_remove_condensates, sr_adapt_cond, len_abunds,
     > out_elnam, out_eps, out_eps0, out_eldust, out_nmol,
     > out_nat, out_nion, out_dustnam, out_cmol, out_dustmass,
     > out_dustvol, out_nHges, out_rhod, out_ngas)
***********************************************************************      
     ...
     >                   DUALcorr,HCOcorr,nat,nion,nmol
      ...
      character(len=200), intent(in) :: sr_elements
      integer :: len_abunds
      real*8, intent(in), dimension(len_abunds) :: sr_abunds          
Cf2py integer intent(hide),depend(sr_abunds) :: len_abunds=shape(sr_abunds,0)      
      logical, intent(in) :: sr_model_eqcond
      integer, intent(in) :: sr_Npoints
      real*8, intent(in) :: sr_Tfast, Temp, Pres, sr_nHmin
      real*8, intent(in) :: sr_nHmax,sr_Mpl,sr_Rpl,sr_gamma
      logical, intent(in) :: sr_phyllosilicates, sr_remove_condensates
      logical, intent(in) :: sr_pick_mfrac, sr_adapt_cond
      integer :: i,counter
      character(len=2), intent(out) :: out_elnam(len_abunds)
      real(kind=8), intent(out) :: out_eps(len_abunds)
      real(kind=8), intent(out) :: out_eps0(len_abunds) 
      real(kind=8), intent(out), ALLOCATABLE :: out_eldust(:)
      real(kind=8), intent(out), ALLOCATABLE :: out_nmol(:) 
      real(kind=8), intent(out) :: out_nat(len_abunds)
      real(kind=8), intent(out) :: out_nion(len_abunds)
      character(len=20), intent(out), ALLOCATABLE :: out_dustnam(:) 
      character(len=20), intent(out), ALLOCATABLE :: out_cmol(:)
      real*8, intent(out), ALLOCATABLE :: out_dustmass(:)
      real*8, intent(out), ALLOCATABLE :: out_dustvol(:)
      real*8, intent(out) :: out_nHges, out_rhod, out_ngas

      call READ_PARAMETER(Temp, Pres, sr_elements, 
     > sr_Mpl, sr_Rpl, sr_Npoints, sr_Tfast,sr_nHmin,sr_nHmax,sr_gamma,
     > sr_model_eqcond, sr_pick_mfrac,sr_phyllosilicates,
     > sr_remove_condensates, sr_adapt_cond)
      call INIT(sr_abunds,len_abunds)
      call INIT_CHEMISTRY
      call INIT_DUSTCHEM
      ALLOCATE( out_eldust(NDUST))

      if (model_dim==0) then
        if (adapt_cond) then
          call ADAPT_CONDENSATES(out_eps,out_eldust,out_nHges,
     > out_rhod,out_ngas)
        else
          call DEMO_CHEMISTRY(out_eps,out_eldust,out_nHges,
     > out_rhod,out_ngas)
        endif  
      ...
      out_nat = nat
      out_nion = nion
      out_dustnam = dust_nam
      out_cmol = cmol
      out_dustmass = dust_mass
      out_dustvol = dust_Vol
      counter = 0
...
***********************************************************************
      SUBROUTINE DEMO_CHEMISTRY(out_eps, out_eldust, out_nHges,out_rhod,
     > out_ngas)
***********************************************************************
     ...
     >                    dust_nel,dust_el,dust_nu
      ...
      real(kind=qp), intent(out) :: out_eps(NELEM)
      real(kind=qp), intent(out) :: out_eldust(NDUST)
      real*8, intent(out) :: out_nHges, out_rhod, out_ngas
